...
func (a *Action) Validate() error {
	...
	if err := requiredFieldsError(requiredField(a.ID, "id"), requiredField(a.Title, "title")); err != nil {
		return err
	}
	if !a.Kind.Valid() {
...
func validateContextIDs(contextIDs []string) error {
	for i, id := range contextIDs {
		if strings.TrimSpace(id) == "" {
			return &ValidationError{Problems: []string{"context[" + itoa(i) + "]: Required value"}}
		}
	}
	return nil
}
...
func validateLabels(labels []Label) error {
	for i, label := range labels {
		if strings.TrimSpace(label.Name) == "" {
			return &ValidationError{Problems: []string{"labels[" + itoa(i) + "].name: Required value"}}
		}
	}
	return nil
}
