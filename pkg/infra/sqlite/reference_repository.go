...
type ReferenceRepository struct{ db *sqlx.DB }

func NewReferenceRepository(db *sqlx.DB) *ReferenceRepository { return &ReferenceRepository{db: db} }
...
func toReferenceDomain(row referenceRow) (*domain.Reference, error) {
	references, err := unmarshalReferenceLinks(row.References)
	if err != nil {
		return nil, err
	}
...
func marshalReferenceLinks(references []domain.ReferenceLink) (string, error) {
	if len(references) == 0 {
		return "[]", nil
	}
...
func unmarshalReferenceLinks(raw []byte) ([]domain.ReferenceLink, error) {
...
func applyReferenceFilters(base string, q domain.ReferenceListQuery, args []any) (string, []any) {
	base += " WHERE 1=1"
	if q.Search != "" {
...
func referenceOrderBy(q domain.ReferenceListQuery) (string, error) {
...
func applyReferenceLimitOffset(base string, q domain.ReferenceListQuery, args []any) (string, []any) {
	if q.Limit > 0 {
		base += " LIMIT ?"
...
func (r *ReferenceRepository) Create(ctx context.Context, reference *domain.Reference) error {
	if err := reference.Validate(); err != nil {
		return err
	}
	references, err := marshalReferenceLinks(reference.References)
	if err != nil {
		return err
	}
	const q = `
...
func (r *ReferenceRepository) Get(ctx context.Context, id string) (*domain.Reference, error) {
	const q = `
	...
	return toReferenceDomain(row)
}

func (r *ReferenceRepository) List(ctx context.Context, q domain.ReferenceListQuery) ([]*domain.Reference, error) {
	...
	`
	args := []any{}
	base, args = applyReferenceFilters(base, q, args)
	ob, err := referenceOrderBy(q)
	if err != nil {
		return nil, err
	}
	base += ob
	base, args = applyReferenceLimitOffset(base, q, args)
	...
	for _, row := range rows {
		reference, err := toReferenceDomain(row)
		if err != nil {
			return nil, err
		}
...
func (r *ReferenceRepository) Update(ctx context.Context, reference *domain.Reference) error {
	if err := reference.Validate(); err != nil {
		return err
	}
	references, err := marshalReferenceLinks(reference.References)
