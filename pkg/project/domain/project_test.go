...
func TestProjectLifecycle(t *testing.T) {
	...
	p, err := NewProject("id", "title", []Goal{{Title: "goal"}}, "desc", nil, now)
...
func TestProjectUpdateValidationErrorIncludesMissingFields(t *testing.T) {
	...
	p, err := NewProject("id", "title", []Goal{{Title: "goal"}}, "desc", nil, time.Now())
	g.Expect(err).ToNot(gomega.HaveOccurred())

	err = p.UpdateDetails("", []Goal{{Title: "goal"}}, "", nil, time.Now())
	g.Expect(err).To(gomega.MatchError("invalid argument: [title: Required value, description: Required value]"))
}
...
func TestProjectGoalTimestampNormalization(t *testing.T) {
	...
	p, err := NewProject("id", "title", []Goal{{Title: "  my goal  ", CompletedAt: &completedAt}}, "desc", nil, now)
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(p.Goals).To(gomega.HaveLen(1))
	...
	err = p.UpdateDetails("title", []Goal{{Title: "goal"}}, "desc", nil, now2)
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(p.Goals[0].CompletedAt).To(gomega.BeNil())
}
...
func TestProjectReferenceValidation(t *testing.T) {
	...
	_, err := NewProject("id", "title", []Goal{{Title: "goal"}}, "desc", []Reference{{Title: "", URL: "https://example.com"}}, time.Now())
	g.Expect(err).To(gomega.MatchError(gomega.ContainSubstring("references[0].title: Required value")))

	_, err = NewProject("id", "title", []Goal{{Title: "goal"}}, "desc", []Reference{{Title: "ref", URL: "notaurl"}}, time.Now())
