...
func unmarshalGoals(raw []byte) ([]domain.Goal, error) {
...
func marshalReferences(references []domain.Reference) (string, error) {
	if len(references) == 0 {
		return "[]", nil
	}
...
func unmarshalReferences(raw []byte) ([]domain.Reference, error) {
	raw = []byte(strings.TrimSpace(string(raw)))
	if len(raw) == 0 {
		return nil, nil
	}
...
func (r *Repository) Create(ctx context.Context, p *domain.Project) error {
	goals, err := marshalGoals(p.Goals)
	if err != nil {
		return err
	}
	references, err := marshalReferences(p.References)
	if err != nil {
		return err
	}
	const q = `
