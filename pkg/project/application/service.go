...
func (s *Service) Create(ctx context.Context, cmd CreateProjectCmd) (*domain.Project, error) {
	...
	p, err := domain.NewProject(id, cmd.Name, cmd.Goal, cmd.Principles, cmd.VisionResult, cmd.Description, now)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, id string, cmd UpdateProjectCmd) (*domain.Project, error) {
	...
	if err := p.UpdateDetails(cmd.Name, cmd.Goal, cmd.Principles, cmd.VisionResult, cmd.Description, s.now()); err != nil {
		return nil, err
	}
