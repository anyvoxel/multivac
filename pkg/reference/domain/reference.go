...
func (r *Reference) Validate() error {
	...
	if err := requiredFieldsError(requiredField(r.ID, "id"), requiredField(r.Title, "title")); err != nil {
		return err
	}
...
func (r *Reference) UpdateDetails(title, description string, references []ReferenceLink, now time.Time) error {
	...
	r.References = normalizeReferences(references)
	r.UpdatedAt = now.UTC()
	return r.Validate()
}

func validateReferences(references []ReferenceLink) error {
...
func normalizeReferences(references []ReferenceLink) []ReferenceLink {
	if len(references) == 0 {
		return nil
	}
