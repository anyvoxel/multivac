...
func TestRepositoryCRUD(t *testing.T) {
	...
	someday, err := domain.NewSomeday("s1", "name", "desc", now)
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(repo.Create(ctx, someday)).To(gomega.Succeed())

	got, err := repo.Get(ctx, "s1")
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(got.ID).To(gomega.Equal("s1"))
	...
	g.Expect(someday.UpdateDetails("name2", "desc2", now2)).To(gomega.Succeed())
	g.Expect(repo.Update(ctx, someday)).To(gomega.Succeed())
	...
	g.Expect(repo.Delete(ctx, "s1")).To(gomega.Succeed())
...
func TestRepositoryListSupportsSearchPaginationAndSort(t *testing.T) {
	...
	alpha, err := domain.NewSomeday("s1", "Alpha", "Search item", now)
	g.Expect(err).ToNot(gomega.HaveOccurred())
	beta, err := domain.NewSomeday("s2", "Beta Search", "desc", now.Add(time.Minute))
	g.Expect(err).ToNot(gomega.HaveOccurred())
	gamma, err := domain.NewSomeday("s3", "Gamma", "other", now.Add(2*time.Minute))
	g.Expect(err).ToNot(gomega.HaveOccurred())
	...
	paged, err := repo.List(ctx, domain.ListQuery{
		Sorts:  []domain.Sort{{By: domain.SomedaySortByName, Dir: domain.SortAsc}},
		Limit:  1,
		Offset: 1,
	})
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(paged).To(gomega.HaveLen(1))
	g.Expect(paged[0].ID).To(gomega.Equal("s2"))

	caseInsensitive, err := repo.List(ctx, domain.ListQuery{Search: "ALPHA"})
	g.Expect(err).ToNot(gomega.HaveOccurred())
	g.Expect(caseInsensitive).To(gomega.HaveLen(1))
	g.Expect(caseInsensitive[0].ID).To(gomega.Equal("s1"))
}
