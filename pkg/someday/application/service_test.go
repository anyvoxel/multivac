...
func TestServiceCreateGeneratesULIDAndUTCFields(t *testing.T) {
	g := gomega.NewWithT(t)
	db, err := sqlx.Open("sqlite3", ":memory:")
	g.Expect(err).ToNot(gomega.HaveOccurred())
	defer func() { _ = db.Close() }()
	ctx := context.Background()
	repo := somedaysqlite.NewRepository(db)
...
func TestServiceConvertFromInboxKeepsIDAndRemovesInbox(t *testing.T) {
	g := gomega.NewWithT(t)
	db, err := sqlx.Open("sqlite3", ":memory:")
	g.Expect(err).ToNot(gomega.HaveOccurred())
	defer func() { _ = db.Close() }()
	...
	someday, err := svc.ConvertFromInbox(ctx, "01J0000000000000000000301", ConvertFromInboxCmd{Title: &overrideTitle, Description: &overrideDesc})
	g.Expect(err).ToNot(gomega.HaveOccurred())
