...
import (
	...
	"github.com/anyvoxel/multivac/pkg/context/application"
...
func toResp(contextObj *domain.Context) contextResp {
...
func (h *Handler) Create(c context.Context, ctx *app.RequestContext) {
	...
	ctx.JSON(stdhttp.StatusCreated, toResp(contextObj))
}
...
func (h *Handler) Get(c context.Context, ctx *app.RequestContext) {
	...
	ctx.JSON(stdhttp.StatusOK, toResp(contextObj))
}
...
func (h *Handler) List(c context.Context, ctx *app.RequestContext) {
	...
	for _, contextObj := range contexts {
		resp = append(resp, toResp(contextObj))
	}
